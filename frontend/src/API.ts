import { Credential, Session, User, Experiment, Drawing, Rating } from "@/types"
...
export default {
    ...
    endExperiment: (data: { experiment_id: number, end_time: string }) => {
        return client.put<Experiment>(`/api/experiments/${data.experiment_id}`, { end_time: data.end_time })
    },
    getExperiments: () => {
    ...
    startSession: (data: { word: string, experiment_id: number }) => {
        return client.post<Session>('/api/sessions', data)
    },
    endSession: ({ session_id, end_time }: { session_id: number, end_time: string }) => {
